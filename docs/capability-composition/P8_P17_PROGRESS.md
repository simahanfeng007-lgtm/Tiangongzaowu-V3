...
- P11 R1A engineering is **COMPLETE** at source freeze
  `318b8bd7fc43c2c27e02ba202cdb3cb4287426c2`. The read-only intake binds
  existing append-only `RunObservation` bytes to an exact
  provider/model/revision and the P4 Proposal/Plan chain. Its 13/13 exact-head
  jobs passed; Ubuntu and Windows P11 jobs each report 71 passed / 0 failed /
  0 skipped and the recorded identity/matrix bytes match across platforms.
  Live provider calls, production Gateway/P19 trace joins and independent
  review are still pending; R0/R1A are not production cutover evidence.
...
| Formal model matrix | Core 80 × 4 models; long-tail 120 × primary and weak model; fault 40 × models | RECORDED CONTRACT + LIVE INTAKE EXACT-HEAD VERIFIED — LIVE EVIDENCE PENDING |
