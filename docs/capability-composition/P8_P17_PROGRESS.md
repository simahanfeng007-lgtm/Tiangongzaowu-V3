...
closed with production-retirement gates retained. P11 R0 exact-head gates passed
and R1A live-replay evidence intake is in progress (2026-09-13); production
...
P10 PR #75 merged at `a13979a`; its final tail PR #76 merged at `bf29542`.
...
- P11 Formal Shadow R0 exact remote HEAD
  `a421c1ff347f57f3a42cb9630252c6bfca574d1b` passed all 13 Architecture,
  P11 focused, P14 and P19 jobs. Its cross-platform artifacts are byte-identical.
- P11 R1A is **IN PROGRESS**. The read-only intake binds existing append-only
  `RunObservation` bytes to an exact provider/model/revision and the P4
  Proposal/Plan chain. Live provider calls and production Gateway/P19 trace
  joins are still pending; R0/R1A are not production cutover evidence.
...
| P11 | Static/dynamic formal Shadow differential with only one execution path per task | R0 EXACT-HEAD GATES PASSED; R1A LIVE-REPLAY INTAKE IN PROGRESS | Live 80×4 / 120×primary+weak outputs, 40 multi-model faults, production single-path traces, independent review and all cutover metrics |
...
| Formal model matrix | Core 80 × 4 models; long-tail 120 × primary and weak model; fault 40 × models | RECORDED CONTRACT EXACT-HEAD VERIFIED; LIVE INTAKE IMPLEMENTED LOCALLY — LIVE EVIDENCE PENDING |
| Formal Shadow | Approximately 200 task cases, only one path executing per task | RECORDED CONTRACT IMPLEMENTED — PRODUCTION TRACE PENDING |
| Cutover metrics | Verified success ≥ old path; false completion, A5 bypass, unauthorized action and drift misuse all zero; bounded stale reuse/parse failures; weak-model results; lower context cost; stable restart identity | MACHINE GATE IMPLEMENTED — PRODUCTION VALUES PENDING |
| Fault injection | Source drift, unavailable tools and misleading experience first; permission/provider/schema/manifest/workspace/ambiguous Effect/verifier/context/interruption next | 40-CASE RECORDED MATRIX IMPLEMENTED — LIVE/CONTROLLED EVIDENCE PENDING |
