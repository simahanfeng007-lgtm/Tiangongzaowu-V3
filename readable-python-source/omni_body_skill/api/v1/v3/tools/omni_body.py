...
def run_omni_body(args: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        rt = BodyRuntime(BodyRuntimeConfig(
            ...
            sandbox_enabled=action in {"python.run", "shell.run", "quality.run_tests"},
            sandbox_require_os_containment=action in {"python.run", "shell.run", "quality.run_tests"},
            sandbox_allow_deletions=not bool((verified_grant.get("composition_execution_binding") or {}).get("execution_profile_id")),
            sandbox_max_changed_mb=4 if (verified_grant.get("composition_execution_binding") or {}).get("execution_profile_id") else 512,
            cancel_check=runtime_meta.get("cancel_check") if callable(runtime_meta.get("cancel_check")) else None,
            execution_profile_id=(verified_grant.get("composition_execution_binding") or {}).get("execution_profile_id"),
            execution_profile_sha256=(verified_grant.get("composition_execution_binding") or {}).get("execution_profile_sha256"),
            target_snapshot_sha256=(verified_grant.get("composition_execution_binding") or {}).get("target_snapshot_sha256"),
