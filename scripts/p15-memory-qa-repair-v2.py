...
def replace_slice(text: str, start_anchor: str, end_anchor: str, replacement: str) -> str:
...
def patch_coordinator(text: str) -> str:
    # P15 explicit L4 attached by the real chat route must carry temporary expiry.
    ...
    if "deadline = expiry_deadline_ms(detection.expiry_kind, created_at_ms)" not in block:
        ...
        if "            expires_at_ms=None," not in block:
...
'''
    return replace_slice(text, "def invalidate_cascade(", "__all__ = [", replacement)
...
def patch_gateway(text: str) -> str:
    if "from datetime import datetime, timezone\n" not in text:
        ...
        text = text.replace(anchor, anchor + "from datetime import datetime, timezone\n", 1)
