...
def replace_top_level_function(text: str, name: str, replacement_lines: list[str]) -> str:
    tree = ast.parse(text)
...
text = replace_once(text, "    decide_task_contract_completion,\n", "", "remove direct completion import")
...
for function_name, replacement_lines in replacements.items():
    text = replace_top_level_function(text, function_name, replacement_lines)
