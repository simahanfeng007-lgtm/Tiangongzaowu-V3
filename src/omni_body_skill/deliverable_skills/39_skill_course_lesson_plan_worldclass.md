...
## 标准流程
1. 调用 `course.lesson_plan.create` 生成交付骨架或计划。
2. 根据用户素材补全内容，不允许空泛占位。
3. 调用 `docx.create` 或对应 core/app action 生成最终交付物。
4. 调用 `qc.course.plan_check` 进行质量门检查。
5. 若 score < 80，调用 `repair.plan` 生成返工计划。
6. 按 high/critical 问题优先返工，再重新运行 `qc.course.plan_check`。
7. 调用 `deliverable.package` 打包交付物、QC报告、源素材和说明。

## 质量标准
使用模板 `course_plan` 对应 rubric。验收必须包含：结构完整、受众明确、证据/细节充分、可执行、可复核、低AI腔。

## 工具调用样例
```json
{
  "action": "course.lesson_plan.create",
  "target": "draft.md",
  "args": {
    "objective": "填写交付目标",
    "audience": "填写受众"
