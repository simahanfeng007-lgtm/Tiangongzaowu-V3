"""Internal source-compiler registry.

P2 contains no native source compilers. P3 will register concrete compilers.
"""
...
class CompilerRegistry:
    def __init__(self, compilers: Mapping[str, SourceCompiler] | None = None) -> None:
        self._lock = RLock()
        self._compilers: dict[str, SourceCompiler] = dict(compilers or {})

    def register(self, source_kind: str, compiler: SourceCompiler, *, replace: bool = False) -> None:
        if not source_kind or not callable(compiler):
            raise ValueError("source_kind and callable compiler are required")
        with self._lock:
            if source_kind in self._compilers and not replace:
                raise ValueError(f"compiler already registered for {source_kind}")
            self._compilers[source_kind] = compiler

    def resolve(self, source_kind: str) -> SourceCompiler | None:
        with self._lock:
            return self._compilers.get(source_kind)
