...
- provenance.kind=real 必须给 evidence_refs（仓库内复现测试）；
...
ROWS: list[tuple] = [
    ...
    ("false_obligation", "text", "file_write", "MEDIUM", "FAIL", "RECORD", "none", "real",
     ["tests/test_codex_write_evidence_recognition.py"], '引号内"验证通过.txt"不得派生文件创建义务', []),
    ("false_obligation", "text", "chat", "MEDIUM", "FAIL", "RECORD", "none", "real",
     ["tests/test_codex_write_evidence_recognition.py"], "前端【连续执行契约】横幅文本不得参与义务推导", []),
    ...
    ("evidence_missing", "effect", "file_write", "HIGH", "FAIL", "RECORD", "none", "real",
     ["tests/test_codex_write_evidence_recognition.py"], "工具 success=true 但无 changed_files/pre-post/verified 证据，模型自报完成", []),
    ...
    ("effect_state_mismatch", "effect", "file_write", "HIGH", "ERROR", "RECORD", "none", "real",
     ["tests/test_completion_gate_head_consistency.py"], "effect head 与 FactLedger 投影不一致（fail-closed）", []),
    ...
    ("effect_state_mismatch", "effect", "file_write", "MEDIUM", "INCONCLUSIVE", "RECORD", "none", "synthetic", [],
     "幂等重写 verified_unchanged_files 合法命中（应 PASS 而非误报）", ["effect.idempotent_target_verified"]),
    # 05 artifact_shell ----------------------------------------------------
    ("artifact_shell", "docx", "office_generation", "HIGH", "FAIL", "RECORD", "none", "real",
     ["tests/test_office_content_gate.py"], "DOCX 仅标题无正文（空壳）", ["docx.min_body_items"]),
    ("artifact_shell", "xlsx", "office_generation", "HIGH", "FAIL", "RECORD", "none", "real",
     ["tests/test_office_content_gate.py"], "XLSX 空表（无非空单元格）", ["artifact.nonempty"]),
    ("artifact_shell", "xlsx", "office_generation", "HIGH", "FAIL", "RECORD", "none", "real",
     ["tests/test_office_content_gate.py"], "XLSX 请求列缺失", ["xlsx.required_columns"]),
    ("artifact_shell", "xlsx", "office_generation", "HIGH", "FAIL", "RECORD", "none", "real",
     ["tests/test_office_content_gate.py"], "XLSX 数据行数不足（表头不计入数据行）", ["xlsx.min_data_rows"]),
    ...
    ("artifact_shell", "pdf", "office_generation", "MEDIUM", "INCONCLUSIVE", "RECORD", "none", "synthetic", [],
     "image-only PDF 文本不可提取（不得伪装 PASS）", []),
    # 06 artifact_placeholder ----------------------------------------------
    ("artifact_placeholder", "xlsx", "office_generation", "MEDIUM", "FAIL", "ALERT", "none", "real",
     ["tests/test_office_content_gate.py"], "所有数据行为相同占位 token（TODO/示例内容）", ["artifact.nonempty"]),
    ...
    ("false_block", "pdf", "office_generation", "MEDIUM", "PASS", "ALERT", "none", "synthetic", [],
     "扫描版合法 PDF（INCONCLUSIVE 不得折算 FAIL）", []),
    ...
    ("ambiguous_side_effect", "effect", "file_write", "HIGH", "FAIL", "RECORD", "none", "real",
     ["tests/test_delivery_outbox.py"], "渠道回执歧义分支经门产出非 completed 决策", []),
    ...
    ("completion_self_deadlock", "effect", "file_write", "HIGH", "PASS", "RECORD", "none", "real",
     ["tests/test_codex_write_evidence_recognition.py"], "本代外层 execution envelope 例外不阻断自身完成提案（PR#41 e173da4）", []),
    ("completion_self_deadlock", "effect", "file_write", "HIGH", "PASS", "RECORD", "none", "real",
     ["tests/test_codex_write_evidence_recognition.py"], "权威写入双稳定信号（observed+verified 同时贡献，PR#41 e173da4）", []),
    # 12 frontend_banner_contamination（回归保持类） ---------------------------
    ("frontend_banner_contamination", "text", "chat", "MEDIUM", "PASS", "RECORD", "none", "real",
     ["tests/test_codex_write_evidence_recognition.py"], "横幅剥离后正常完成不回归（PR#41 94816b7）", []),
    # 13 quoted_data_contamination（回归保持类） -------------------------------
    ("quoted_data_contamination", "text", "file_write", "MEDIUM", "PASS", "RECORD", "none", "real",
     ["tests/test_codex_write_evidence_recognition.py"], "引号剥离后正常文件任务完成不回归（PR#41 fc834c2）", []),
    # 14 artifact_only_delivery（回归保持类） ----------------------------------
    ("artifact_only_delivery", "docx", "office_generation", "MEDIUM", "PASS", "RECORD", "none", "real",
     ["tests/test_execution_chain_efficiency_fixes.py"], "空正文+有产物→机器合成交付文案（PR#41 b6b032a 修B）", []),
    ("artifact_only_delivery", "xlsx", "office_generation", "MEDIUM", "PASS", "RECORD", "none", "real",
     ["tests/test_execution_chain_efficiency_fixes.py"], "产物-only 不再整单失败", []),
    # 15 memory_cognition_task_closeout（回归保持类） ---------------------------
    ("memory_cognition_task_closeout", "repository", "cognition", "HIGH", "PASS", "RECORD", "none", "real",
     ["tests/test_life_cognition_execution_wiring.py"], "认知任务经执行回路完成并写回 proposed_relations（PR#41 5c4392f）", []),
    ("memory_cognition_task_closeout", "repository", "cognition", "MEDIUM", "PASS", "RECORD", "none", "real",
     ["tests/test_life_cognition_execution_wiring.py"], "noop 带原因码 + 停机恢复叙事事件（PR#41 5c4392f）", []),
    ("memory_cognition_task_closeout", "repository", "cognition", "MEDIUM", "PASS", "RECORD", "none", "real",
     ["tests/test_memory_unification_landings.py"], "工作指纹/心灵日记/镜像刷新记忆归一（PR#41 d2f2ccd）", []),
    ...
    ("artifact_shell", "xlsx", "office_generation", "LOW", "PASS", "RECORD", "none", "synthetic", [],
     "表头+3行数据满足至少3行要求（happy）", ["xlsx.min_data_rows"]),
