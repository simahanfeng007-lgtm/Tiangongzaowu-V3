...
class GoldenBlockCase(RepairLoopE2EBase):
    ...
    def test_g10_block_failed(self) -> None:
        """G10 (final form). BLOCK is CONTRACT-RESERVED and currently
        UNREACHABLE: the deterministic Repair Policy v1 has no BLOCK
        production branch (REPAIR/WAIT/RECONCILE/REVIEW only, unknown
        falls to REVIEW), and no operator-BLOCK authority exists in
        this plane version. Therefore G10 certifies exactly the two
        real semantics:
        G10-A — a fabricated BLOCK (even one carrying the plane's own
        policy identity and full evidence/readiness bindings) is
        REJECTED by the Gate's Store-authority binding (and the Gate
        REQUIRES the reader whenever a disposition is consumed).
        G10-B — on a CLEAN fixture (real FAIL record + FAIL readiness,
        NO process_readiness, Store current disposition asserted to be
        None — nothing pending), the terminal FAILED authority
        decision comes from the CompletionGate alone."""
        ...
        # ---------- G10-A: fabricated BLOCK rejected ----------
        readiness = self._reverify()
        dispositions = self.coordinator.process_readiness(
            plan=self.plan, readiness=readiness
        )
        ...
        fabricated_block = VerificationDisposition(
        ...
        # fail-closed: a disposition without a reader is refused outright
        with self.assertRaisesRegex(
            ...
            gate.evaluate(
                ...
                active_plan=self.plan,
                verification_readiness=readiness,
                verification_disposition=fabricated_block,
            )
        # with the reader, the never-persisted BLOCK is not authoritative
        with self.assertRaisesRegex(
            Exception,
        ...
        # ---------- G10-B: clean fixture → Gate FAILED ----------
        clean = object.__new__(type(self))
        ...
        try:
            ...
            current = clean.gateway_store.get_current_verification_disposition(
                ...
                generation=GEN,
                verification_plan_id=clean.plan.verification_plan_id,
                readiness_sha256=clean_readiness.readiness_sha256,
            )
            self.assertIsNone(current)
            decision = CompletionGate(
                ...
                clean.fact_ledger,
                head_state_reader=(
                    clean.gateway_store.get_effect_head_state
                ),
            ).evaluate(
                _plan_bound_requirements(clean),
                artifacts=tuple(clean.manifests),
                active_plan=clean.plan,
                verification_readiness=clean_readiness,
                disposition_authority_reader=(
                    clean.gateway_store.get_verification_disposition_by_id
                ),
            )
...
class GoldenBlockCase(RepairLoopE2EBase):
    ...
    def test_g10_block_failed(self) -> None:
        ...
        G10-A — a fabricated BLOCK (even one carrying the plane's own
        policy identity and full evidence/readiness bindings) is
        REJECTED by the Gate's Store-authority binding.
        G10-B — a real verification terminal failure produces the
        FAILED outcome through the CompletionGate alone."""
        from contracts.verification import VerificationDisposition
        from total_gateway.verification_repair_policy import (
            DEFAULT_POLICY,
            POLICY_VERSION,
        ...
        # G10-A: fabricated BLOCK with PERFECT policy identity and
        # bindings — it was never persisted by the Store, so the Gate's
        # disposition-authority reader rejects it outright.
        fabricated_block = VerificationDisposition(
            verification_disposition_id="vds_" + "1" * 64,
            request_id=self.request.request_id,
            run_id=self.run.run_id,
        ...
        # G10-B: real terminal verification failure (failing readiness,
        # NO disposition — nothing pending) → the terminal FAILED
        # authority decision comes from the CompletionGate alone.
        decision = gate.evaluate(
            _plan_bound_requirements(self),
            artifacts=tuple(self.manifests),
            active_plan=self.plan,
