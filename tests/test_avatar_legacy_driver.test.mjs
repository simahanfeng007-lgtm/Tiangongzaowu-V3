...
test("legacy driver: 自然站姿为生物学中立位（JOSR 2022 标定：外展4.5°+内旋9°/肘屈15.5°/掌心朝内映射）", () => {
  ...
  // 前臂：肘前屈 ~10° + 外翻 10.3-10.4°（右 0.300, 0.180, ~0；左 0.300, -0.200, -0.020）
  assert.ok(Math.abs(bones.get("rightLowerArm").rotation.x - 0.300) < 0.05, `右前臂 x 应 ≈0.30，实际 ${bones.get("rightLowerArm").rotation.x}`);
  assert.ok(Math.abs(bones.get("rightLowerArm").rotation.y - 0.180) < 0.05, `右前臂 y 应 ≈0.18，实际 ${bones.get("rightLowerArm").rotation.y}`);
  assert.ok(Math.abs(bones.get("rightLowerArm").rotation.z) < 0.05, `右前臂 z 应 ≈0，实际 ${bones.get("rightLowerArm").rotation.z}`);
  // 手腕：掌心朝身体（-0.4233, 0, 0），手指沿前臂（0°）
  assert.ok(Math.abs(bones.get("rightHand").rotation.x + 0.4233) < 0.05, `右手腕 x 应 ≈-0.42，实际 ${bones.get("rightHand").rotation.x}`);
