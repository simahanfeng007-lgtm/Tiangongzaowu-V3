...
def _materialized_one_world():
    ...
    tools = compile_tool_capability_contribution(frame, world_cut, tool_world)
    ...
    snapshot = materialize_one_world_state(
        materializer, data, (software, tools, methods)
    )
...
def test_exact_frame_contributions_share_one_binding_and_are_non_authorizing() -> None:
    snapshot, _store, frame, world_cut, tools, methods = _materialized_one_world()
...
def test_domain_binding_rejects_other_frame_and_generic_runtime_frame() -> None:
    world_cut = cut()
    frame, _graph = graph_for(world_cut)
    ...
    with pytest.raises(
        ...
        compile_tool_capability_contribution(generic, world_cut, tool_world)
...
def test_capability_context_uses_existing_slot_and_preserves_all_identities() -> None:
    snapshot, _store, _frame, _cut, tools, _methods = _materialized_one_world()
    ...
    assert (
        f"world_state_ref={snapshot.state_ref.record_id}@{snapshot.state_ref.sha256}"
        in text
    )
